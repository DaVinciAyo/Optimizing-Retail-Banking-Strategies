...
# Date conversion
bank["CustomerDOB"] = bank["CustomerDOB"].str.replace("-", "/")
...
# Create 'Age' column
def calculate_age(bank):
    bank["age"] = bank["TransactionDate"].dt.year - bank["CustomerDOB"].dt.year
    return bank
def adjust_year(bank):
    if bank.year > 2016:
        bank = bank.replace(year = bank.year - 100)
    return bank
